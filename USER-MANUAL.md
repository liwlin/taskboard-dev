...
当存在原生子代理后备调度时，progress 还会输出 `subagent_fallback_available`、`subagent_fallback_packet_available`、`subagent_fallback_packet_file`、`subagent_prompt_count`、`subagent_prompt_roles`、`subagent_dispatched_roles`、`subagent_pending_roles` 和 `subagent_dispatch_state_file`，用于让 T0 或集成层从 `.taskboard/t0/subagent-fallback.json` / `.taskboard/t0/subagents.json` 恢复完整派发状态。

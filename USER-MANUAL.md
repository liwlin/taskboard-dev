...
默认只需要 1 个终端，只启动 T0。用户把目标交给 T0，T0 负责启动、恢复、监督 T1/T2/T3，直到目标完成或触发停止门。你不需要开 4 个终端，也不需要分别给 T1/T2/T3 定义角色。目标指令是长任务 loop 的核心输入；如果客户端有专门的 goal/target 参数，就把 `目标` 文本放进去，否则直接粘贴在 `/taskboard-dev T0` 前面。

...
当存在原生子代理后备调度时，progress 还会输出 `subagent_fallback_available`、`subagent_fallback_packet_available`、`subagent_fallback_packet_file`、`subagent_prompt_count` 和 `subagent_prompt_roles`，用于让 T0 或集成层从 `.taskboard/t0/subagent-fallback.json` 恢复完整派发 prompts。

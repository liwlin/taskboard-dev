# taskboard-dev v4.2 用户手册

三终端 TASKBOARD 驱动开发工作流 — 架构师、审核者、执行者协作，基于文件名即状态的零轮询开销设计。v4.2 面向 Claude Code / Codex 的现代长任务能力：loop、目标/target、后台执行、resume 和工具检查点。默认原则是“能自动做就自动做”，只有真正的停止门才需要人确认。
...
8. [长任务运行、目标与 /loop](#8-长任务运行目标与-loop)
9. [内置命令](#9-内置命令)
10. [审核分级](#10-审核分级)
...
打开 3 个 agent CLI 终端，**先写目标指令，再执行角色命令**。目标指令是长任务 loop 的核心输入；如果客户端有专门的 goal/target 参数，就把 `目标` 文本放进去，否则直接粘贴在 `/taskboard-dev T{N}` 前面。
...
这些边界**由当前 agent 自己遵守**（SKILL.md 里写明），不是完全依赖 permission 强制。v4.2 的重点不是让人逐步确认，而是把“需要确认”的范围缩小到停止门：产品决策、破坏性/共享状态操作、凭据/付费/隐私风险、重复失败、范围扩张。常规任务流转、build/test、代码修改、验证和提交都应自动完成。
...
优先使用客户端支持的“目标/target/goal + 长任务运行”能力，让每个角色持续工作到目标完成或触发停止门。命令名按 Claude Code / Codex 的实际版本调整，核心是使用快速开始中的目标指令模板，并确保每个目标包含：

- 角色要持续达成的结果
- 自动执行范围
- 停止门列表
- `/taskboard-dev T{N}` 或 `/taskboard-next` 入口
...
不一定。v4.2 默认让 T3 自动重读最小读取集并继续，保留 v1 中已验证且被 T1 标为 keep 的经验。仅在以下情况重启或暂停：
...
T2 审核发现真正的停止门时，在 Current Instruction 中写入决策选项（Options A/B/C + 推荐），然后改名为 `T1-待决策`。T1 轮询到后只在确实需要产品/安全/破坏性操作选择时呈现给用户；如果只是实现细节或可安全默认的技术取舍，T1 应自主决策、记录到 STATE.md，并升版本继续。

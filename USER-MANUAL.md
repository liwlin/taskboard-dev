...
`taskboard_progress.py` 会从 `.taskboard/t0/latest.json` 或 append-only event log 恢复 `subagent_fallback_available`、`subagent_prompt_count` 和 `subagent_prompt_roles`。因此 T0 重启或 latest snapshot 丢失后，仍能知道当前有原生子代理后备调度路径，不会退回让用户分别管理 T1/T2/T3。

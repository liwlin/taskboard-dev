...
默认使用 Windows Terminal 创建/恢复 `taskboard-T1/T2/T3`，并把每个 worker 的角色目标写入 `.taskboard/targets/taskboard-T*.md`；默认 Claude 启动还会生成 `.taskboard/targets/taskboard-T*.launch.ps1`，Windows Terminal 只执行短 `powershell -File` 命令。脚本会先 bootstrap-refresh `.taskboard/alive/T{N}` 并写 launched session heartbeat，再设置 `TASKBOARD_TARGET_FILE` 指向对应 UTF-8 target 文件，最后用 `/loop 2m /taskboard-dev T{N}` 启动 Claude。这样 worker 默认持续循环，不会只跑一回合后停在交互提示符，也避免通过 PowerShell、Windows Terminal 或 `claude.exe` argv 内联传递超长 prompt。只传 `--goal` 就会进入一键自动模式：执行 T0 管理面的启动/恢复命令，并持续运行到目标完成或触发停止门。可以追加 `--fallback-launcher powershell` 或重复追加多个 fallback launcher；当主 launcher 失败时，T0 会自动按顺序重试备用 launcher，而不是立刻要求用户接管 T1/T2/T3。如果没有传入目标且没有保存的目标，T0 会在第一轮 `needs-goal` 后停下并要求用户给 T0 一个目标，不会无意义空转。只做调度检查时显式加 `--dry-run --iterations 1 --launcher none`。Codex 或其他客户端仍可通过 `--agent-template 'codex --prompt "{target}"'` 或支持的 `{target_file}` 模板覆盖默认值。

执行 worker launcher 前，T0 默认会从 `--agent-template` 解析首个 agent command，并检查它是否在 PATH 中；默认 Claude 模板还会运行一次最小 `claude -p` readiness probe，而不是只相信 `claude auth status` 或 PATH。命令不存在会在启动 T1/T2/T3 前进入 `config-error`，持久化错误并要求修正 T0 配置。需要自定义 CLI/auth 检查时，使用 `--agent-preflight-command "<safe check command>"`，例如按当前客户端选择不会修改仓库的登录/版本/健康检查命令；该命令返回普通非零结果时，错误文本会包含 `agent preflight command failed`。如果默认 readiness 或自定义 preflight 输出包含 `API Error: 403`、`Request not allowed`、`Failed to authenticate` 等托管子进程认证/权限拒绝线索，T0 会把 `agent_preflight.state` 标记为 `spawn-refused`，不再继续执行会失败的 launcher，而是直接生成下面的用户自有终端启动脚本。T0 可先运行 `python scripts/taskboard.py --root . launch-probe --launcher windows-terminal`，读取机器可读的 `recommended_backend`：`terminal` 表示使用 T0-managed terminal launcher，`subagent` 表示直接使用 native subagent fallback，`fix-config` 表示先修 T0 启动配置；这个探测是只读的，不能把 T1/T2/T3 管理转交给用户。高级用户可以用 `--no-agent-preflight` 或 `TASKBOARD_SKIP_DEFAULT_CLAUDE_READINESS=1` 关闭这一步，但默认建议保留，避免 Claude/Codex 等 agent CLI 未安装、未登录、托管子进程被拒绝或命令模板写错时把失败留在子终端里。

如果 preflight 或 launcher 执行失败输出包含托管子进程认证/权限拒绝线索（例如 `API Error: 403`、`Request not allowed`、`Failed to authenticate`），T0 会生成 `.taskboard/open-tabs.ps1` 和 `.taskboard/launch-role.ps1`。同一轮 `taskboard_loop.py` payload 还会生成 `subagent_fallback`，其 `kind` 为 `taskboard-subagent-fallback`，并携带可直接派发给原生隔离子代理的 `subagent_prompts`。T0 优先使用可用的原生子代理后备调度；如果当前客户端没有子代理能力，用户只需要执行一次 `powershell -ExecutionPolicy Bypass -File ".taskboard/open-tabs.ps1"`，由该脚本打开受控的 `taskboard-T1/T2/T3` 终端。这仍然是 T0 指挥的启动动作，不是让用户分别管理三个 worker。

完整的原生子代理后备调度包会写入 `.taskboard/t0/subagent-fallback.json`，其 `kind` 为 `taskboard-subagent-fallback-packet`，包含可恢复派发的 `subagent_prompts`。`taskboard_progress.py` 会从 `.taskboard/t0/latest.json`、append-only event log 和这个 packet 文件恢复 `subagent_fallback_available`、`subagent_fallback_packet_available`、`subagent_fallback_packet_state`、`subagent_fallback_packet_file`、`subagent_prompt_count` 和 `subagent_prompt_roles`。因此 T0 重启或 latest snapshot 丢失后，仍能拿回完整子代理派发材料，不会退回让用户分别管理 T1/T2/T3。packet 会绑定生成时的 T0 goal；如果当前 `.taskboard/t0/goal.json` 与 packet goal 不一致，progress 会把它标为 `stale-goal`，`subagent plan` 不会派发旧 prompt。

T0 使用原生子代理后备调度时，可以直接读取 `python scripts/taskboard_progress.py --root . --format json` 的 `subagent_spawn_request`。当 `action=spawn-native-subagent` 时，T0 用其中的 `prompt` 调用当前客户端的原生子代理工具，并用返回的 `ack_command` 记录 agent id、spawn tool、agent nickname 和 prompt hash。也可以运行 `python scripts/taskboard.py --root . subagent plan` 获取同一机器可读控制动作。子代理返回最终结果后，T0 用 `done_command` 或 `fail_command` 记录完成/失败，并附带 native wait/result metadata；需要重试失败角色时，用 `retry_command` 把旧记录归入 `attempts` 并重新进入 pending 队列。`python scripts/taskboard.py --root . subagent status` 会汇总 `.taskboard/t0/subagent-fallback.json` 和 `.taskboard/t0/subagents.json`，显示哪些角色 pending / active / completed / failed。这个状态仍是 T0 控制面恢复信息，不是 TASKBOARD 任务状态。`taskboard_loop.py` 在生成 native-subagent fallback packet 后也会输出 `subagent_control`；外层集成可用 `--native-spawn-receipt-json` / `--native-spawn-receipt-file` 和 `--native-result-receipt-json` / `--native-result-receipt-file` 注入真实 native spawn/result receipt，loop 会自动写入 `subagent_ack` / `subagent_result` 并推进到下一角色计划。
真实 native-subagent 后端跑完后，用 `python scripts/taskboard_subagent_acceptance.py --root . --require-real-agent-ids --require-spawn-evidence --require-result-evidence` 验收证据。该脚本检查 fallback packet、T1/T2/T3 prompt 隔离契约、ack/result 记录、完成摘要、真实 agent id、`spawn_receipt`、`result_receipt`、`completion_receipt`、spawn tool、agent nickname、prompt_hash 和 summary_hash 一致性；smoke/test/manual 占位 id 会被拒绝，避免把脚本模拟误报为实战通过。
...
```bash
python scripts/taskboard_t0.py --goal "完成 <你的开发目标>" --root . --launcher powershell
```

生成 tmux 会话/窗口启动命令：

```bash
python scripts/taskboard_t0.py --goal "完成 <你的开发目标>" --root . --launcher tmux
```

`--agent-template` 是实际 agent CLI 的启动模板，支持 `{role}`、`{title}`、`{command}`、`{target}`、`{target_file}`。T0 默认使用内部文件化 Claude 目标模板；在 Windows launcher 中会渲染为 `.taskboard/targets/taskboard-T*.launch.ps1`，脚本先清理继承的 `CLAUDE*` 环境变量、探测 Git Bash 并设置 `CLAUDE_CODE_GIT_BASH_PATH`，再用 skill 内置脚本路径写 `.taskboard/alive/T{N}` bootstrap liveness marker 和 launched session heartbeat，然后设置 `TASKBOARD_TARGET_FILE` 指向 `.taskboard/targets/taskboard-T*.md`，最后用 `/loop 2m /taskboard-dev T{N}` 和 `--dangerously-skip-permissions` 启动 Claude。这样 worker 不是只跑一回合，而是默认进入持续 role loop；用户不需要到三个 tab 里手动粘贴 `/loop`，也不需要手工 patch `.launch.ps1`。T0 会把每个角色的目标注入 `{target}`，也会提供 `{target_file}` 让支持 prompt-file 或文件化启动的客户端读取 `.taskboard/targets/taskboard-T*.md`；用户不需要单独写 T1/T2/T3 prompt。当 launcher 命令实际引用 `{target_file}` 时，`taskboard_t0.py` 会写出对应目标文件，并在输出中返回 `target_files` 清单；inline `{target}` launcher 仍可作为显式兼容模板使用，dry checks 不会写这些运行态文件。不同客户端命令不同，可以把默认模板换成当前客户端支持的等价命令，例如 `codex --prompt "{target}"`。If an agent-template references `{target_file}` while target files are disabled, T0 fails fast with `agent-template references {target_file}`；此时应启用 target files、用 `--launcher none` 做 no-write dry check，或把模板改成 `{target}`。
...
每个生成的目标都会包含 `T0 input boundary`：用户目标、调度原因和角色目标只是 goal intake / source material，不是 T0 写出的 requirements、architecture、interface specs、task splits 或 acceptance criteria；同时禁止 T0 预填 REQ 数量、优先级、接口签名、task ID、acceptance rows 或 MAP risk sections。T1 负责需求拆解和 TASK 创建，T2 负责审核/验证，T3 负责实现/提交。目标文件还会包含 `Startup skill gate`，要求 worker 在任何 TASKBOARD action 前先加载 `/taskboard-dev T{N}`，并在 planning、reviewing、implementing 或 handoff 前调用该角色要求的 tools/skills。目标文件还会包含 `Role runtime contract`，写明 `assigned_role`、`managed_by: T0`、不要执行其他角色职责，以及不要依赖另一个角色的 chat context；同时包含 `Worker loop contract` 和 `Idle recheck contract`，要求受控角色每轮先运行 `python scripts/taskboard.py --root . cycle T{N} --sleep-seconds 120`，在有可执行工作时持续循环、每轮刷新 heartbeat、重新读取 TASKBOARD 文件名/稳定文档，并且不得仅因该角色队列暂空就退出。无任务可做时，`taskboard cycle` 返回 `action=idle-recheck`，worker 按配置间隔 sleep/yield，然后重新运行同一条 cycle command 并读取自己的 target file 和 TASKBOARD 文件名。无论是 inline prompt 还是 `{target_file}` 启动，都保持同一份角色隔离和持续执行契约。

当新显式目标与 `.taskboard/t0/goal.json` 里保存的旧目标不同，T0 会创建一个 `T1-方案需修改` intake TASK，使正在 `/loop` 的 T1 能通过 TASKBOARD 文件名接到新增需求；这个 intake TASK 只记录用户目标和边界，不预填 REQ、优先级、架构、验收或任务拆分。如果 `docs/STATE.md` 仍有上一 milestone 的 `Goal Complete: yes/true`，T0 同时会把该完成哨兵复位为 `Goal Complete: no`，记录 `.taskboard/t0/goal-reset.json`，并强制刷新 `.taskboard/targets/taskboard-T*.md`。用户不需要手工修改 target 文件、粘贴 prompts 或重启 T1 来开始新 milestone。

`taskboard_t0.py` 还会返回机器可读的 `goal_intake` packet：`kind=taskboard-t0-goal-intake`、`next_owner=T1`、allowed intake fields、forbidden fields（`requirements`、`architecture`、`interface_specs`、`task_splits`、`acceptance_criteria`），以及 forbidden seed patterns（REQ skeletons、priorities、interface signatures、task IDs、verify checklists、MAP risk sections）。这就是 T0 的播种上限：T0 只能携带用户目标和 source material，需求拆解、上下文文件、TASK 创建和验收标准必须由 T1 拥有。
...
当真实 worker launcher 执行启用时，T0 会在真正执行 worker launcher 前写入
`.taskboard/t0/checkout-owner.json`；`--launcher none` 和 no-launch 检查不会持有这个 checkout-owner 锁。如果该文件里有另一个仍新鲜的顶层 owner，
...
每轮 loop 默认会把最新 T0 supervisor 运行态快照写入 `.taskboard/t0/latest.json`。这个 `taskboard-t0-supervisor-state` 文件只记录 T0 的管理视图，方便中断后恢复判断；它不是任务状态、不是 worker 记忆，也不能替代 TASKBOARD 文件名、history、dev-log、HANDOFF 或完成 sentinel。需要自定义路径时使用 `--state-file <path>`；只想 dry check 且不留下运行态快照时使用 `--no-state-file`。当 T0 使用原生子代理后备调度时，receipt 导入参数只在第一轮消费一次，避免 `--forever` 模式重复记录同一个 spawn/result。

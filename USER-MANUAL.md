...
长时间无人值守时，可以让 watchdog 进入 bounded guardian 模式，重复检查并只恢复 T0 自己：

```bash
python scripts/taskboard_watchdog.py --root . --guardian --execute
```

该命令返回 `taskboard-t0-guardian`，用于降低人工重复运行 watchdog 的需求。它仍然只执行 T0 resume command，不直接启动或管理 T1/T2/T3 的工作内容。

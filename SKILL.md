...
- If `taskboard_start.py --auto` receives Ctrl-C / `KeyboardInterrupt`, it returns 130 and reports `taskboard-t0-interruption`, `state=interrupted`, `resume_command`, and `user_action`. The resume command restarts T0 with the same launcher/template/lease/interval configuration; the user still does not manage T1/T2/T3 directly.

...
Use `scripts/taskboard_progress.py --root .` for a concise user-facing T0 progress summary. It reports the goal, T0 state, next managed role, current task, assignment state, whether user action is required, text `queue_metrics_active_count` / `queue_metrics_stalled_count` / `queue_metrics_role_counts` / `queue_metrics_next_role` lines, and JSON `queue_metrics` for active tasks, stalled tasks, T1/T2/T3 queue counts, and the next controlled role; it does not ask the user to manage T1/T2/T3. When a stop gate is active, progress includes `decision_command` so T0 can record the user's answer and resume T1 without making the user manage TASKBOARD mechanics.

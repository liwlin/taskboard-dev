...
- Use `scripts/taskboard_start.py --goal "<user goal>" --auto` as the one-command user entry. `--auto` executes T0 manager launch/recovery commands and runs until completion by default. It forwards the same latest snapshot, append-only event log, launch lease, and per-role target behavior as `taskboard_loop.py`, and persists `auto_mode` plus `starter_mode` into latest snapshot so `taskboard_progress.py` can confirm recovery is from the one-command automatic entry. For bounded dry verification, use `--auto --iterations 1 --launcher none`.
...
Use `scripts/taskboard_progress.py --root .` for a concise user-facing T0 progress summary. It reports the goal, T0 state, next managed role, current task, assignment state, whether user action is required, text `queue_metrics_active_count` / `queue_metrics_stalled_count` / `queue_metrics_role_counts` / `queue_metrics_next_role` / `event_count` / `latest_event_state` / `latest_event_next_role` / `latest_event_task` / `latest_event_completion_ready` / `completion_ready` / `completion_audit_state` / `completion_missing_evidence` lines, and JSON `queue_metrics` plus latest event plus completion audit for active tasks, stalled tasks, T1/T2/T3 queue counts, the next controlled role, the last T0 supervisor event, and missing completion evidence; it does not ask the user to manage T1/T2/T3. When a stop gate is active, progress includes `decision_command` so T0 can record the user's answer and resume T1 without making the user manage TASKBOARD mechanics.

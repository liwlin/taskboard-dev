...
- Use `scripts/taskboard_start.py` as the one-command user entry. It forwards the same latest snapshot, append-only event log, launch lease, and per-role target behavior as `taskboard_loop.py`, so the recommended T0 entry preserves the same audit/recovery evidence.

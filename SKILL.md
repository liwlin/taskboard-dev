...
- Use `scripts/taskboard_start.py --goal "<user goal>" --auto` as the one-command user entry. `--auto` executes T0 manager launch/recovery commands and runs until completion by default. It forwards the same latest snapshot, append-only event log, launch lease, and per-role target behavior as `taskboard_loop.py`, so the recommended T0 entry preserves the same audit/recovery evidence. For bounded dry verification, use `--auto --iterations 1 --launcher none`.
...
- Each loop iteration appends a compact event to `.taskboard/t0/events.jsonl` by default. This `taskboard-t0-supervisor-event` log is append-only audit/recovery evidence for T0 dispatch, queue, session, assignment, and action summaries across runs; it is not TASKBOARD state or worker memory. Use `--event-log-file <path>` to choose another path, or `--no-event-log` for no-write dry checks.
- T0 stops only when the active TASK queue is empty and `docs/STATE.md` contains `**Goal Complete**: yes` or `Goal Complete: yes`. Empty queue without that completion sentinel means the goal is still incomplete and T0 should wake T1 to create or revise TASK files. `--forever` runs until completion or interruption; `--no-stop-on-complete` is only for post-completion monitoring/debugging.
...
Use `scripts/taskboard_progress.py --root .` for a concise user-facing T0 progress summary. It reports the goal, T0 state, next managed role, current task, assignment state, and whether user action is required; it does not ask the user to manage T1/T2/T3. When a stop gate is active, progress includes `decision_command` so T0 can record the user's answer and resume T1 without making the user manage TASKBOARD mechanics.

...
# TASKBOARD-Driven Development v4.5.36

T0-managed collaborative development. The user gives T0 one goal, and T0 manages the T1 architect/scheduler, T2 reviewer/verifier, and T3 executor loops until the goal is complete or a stop gate is hit. Status is still encoded in filenames. Polling still uses Glob with zero file content reads. The read-only context layer remains the cross-session memory. v4.5.36 keeps the v4 task file protocol, the compact `taskboard.py` CLI facade, completion/subagent/boundary smoke checks, checkout-owner launch guarding that ignores `launcher none`, live milestone acceptance, native-subagent dispatch/result plans, CLI-imported T0 loop subagent control receipts, progress-level cold-resume readiness evidence, a real-project cold-resume acceptance gate, a framework readiness audit that now keeps real native-subagent execution as a separate field gap, and an overnight field-run recorder with status/next-action/readiness/saved-goal or explicit-goal T0-prepare guidance and placeholder-free overnight start next commands while preserving the stricter T0 seeding boundary that forbids T0 from pre-filling REQ skeletons, priorities, interface signatures, task IDs, acceptance rows, or MAP risk sections. This field-fix version also converts changed user goals into a queue-visible T1 intake TASK, hardens the default Claude worker launcher by clearing nested `CLAUDE*` environment variables, detecting Git Bash, using bundled skill script paths for bootstrap heartbeats, passing `--dangerously-skip-permissions`, and explicitly forbidding workers from canceling the client `/loop` for role-local idleness. Explicit new goals still reset stale `Goal Complete: yes` sentinels before target regeneration so a finished milestone cannot block the next milestone. When terminal workers are refused, progress now exposes a goal-checked `subagent_spawn_request` so a parent T0 can call native subagents directly without copying prompts, while stale fallback packets are blocked if their saved goal differs from the current T0 goal. This version also ignores this skill repo's own generated TASKBOARD docs so real field runs can create `docs/taskboard/` recovery evidence without polluting source control.
...
11. If you are T1/T2/T3 and `TASKBOARD_TARGET_FILE` is set, read that UTF-8 target file before any TASKBOARD action. If the environment variable is absent, read the standard `.taskboard/targets/taskboard-T{N}.md` file when present. The target file is the T0-managed inbox for this role; it is not task state and does not override the role reference.
12. Confirm once: "T{N} {role} ready. Run mode: {autonomous/supervised}. Review mode: {full/standard/assisted/manual}. Active tasks: {count}. Stop gates: product/destructive/credential/repeated-failure/scope." For T0, confirm: "T0 orchestrator ready. User goal: {goal}. Managed roles: T1/T2/T3. Stop gates: product/destructive/credential/repeated-failure/scope."
...
- **Never auto-exit on an empty queue**: a role MUST NOT suggest leaving `/loop` or cancel the client `/loop` just because its own queue is currently empty. An empty queue is normal — another role may hand off a task minutes later. The ONLY conditions for suggesting exit are: (1) the user explicitly says stop/pause; (2) the entire project is complete (all tasks archived, dev-log/HANDOFF written, milestone declared done); (3) the session is about to hit its context limit and needs a clean restart.

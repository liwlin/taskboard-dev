...
└── scripts/
    ...
    ├── taskboard_e2e_smoke.py       # 验证 T0 目标到 worker 认领、归档和 completion-ready 的闭环
    ├── taskboard_cold_resume_smoke.py # 验证隔夜 fresh worker 从 TASKBOARD 恢复主题
    ├── taskboard_cold_resume_acceptance.py # 验收真实冷恢复证据
    ├── taskboard_framework_readiness.py # 汇总全流程框架 readiness 与剩余 field gaps
    ├── taskboard_overnight_field_run.py # 记录真实隔夜 field run 的 start/resume/verify 证据
    ├── taskboard_t0_boundary_smoke.py # 验证 T0 dry-run 不写 worker-owned artifacts
    ├── taskboard_subagent_smoke.py  # 验证 T0 native-subagent 后备调度 ack/done/retry 闭环
    ├── taskboard_subagent_acceptance.py # 验收真实 native-subagent 派发证据
    ├── taskboard_live_milestone_acceptance.py # 验收真实 T0-managed milestone 证据
...
```text
dist/taskboard-dev-v4.5.36.tar.gz
dist/taskboard-dev-v4.5.36.zip
```
...
默认使用 Windows Terminal 创建/恢复 `taskboard-T1/T2/T3`，并把每个 worker 的角色目标写入 `.taskboard/targets/taskboard-T*.md`；默认 Claude 启动还会生成 `.taskboard/targets/taskboard-T*.launch.ps1`，Windows Terminal 只执行短 `powershell -File` 命令。脚本会先清理继承的 `CLAUDE*` 环境变量、探测 Git Bash 并设置 `CLAUDE_CODE_GIT_BASH_PATH`，再用 skill 内置脚本路径 bootstrap-refresh `.taskboard/alive/T{N}` 并写 launched session heartbeat，然后设置 `TASKBOARD_TARGET_FILE` 指向对应 UTF-8 target 文件，最后用 `/loop 2m /taskboard-dev T{N}` 和 `--dangerously-skip-permissions` 启动 Claude。这样 worker 默认持续循环，不会只跑一回合后停在交互提示符，也避免通过 PowerShell、Windows Terminal 或 `claude.exe` argv 内联传递超长 prompt。只传 `--goal` 就会进入一键自动模式：执行 T0 管理面的启动/恢复命令，并持续运行到目标完成或触发停止门。可以追加 `--fallback-launcher powershell` 或重复追加多个 fallback launcher；当主 launcher 失败时，T0 会自动按顺序重试备用 launcher，而不是立刻要求用户接管 T1/T2/T3。如果没有传入目标且没有保存的目标，T0 会在第一轮 `needs-goal` 后停下并要求用户给 T0 一个目标，不会无意义空转。只做调度检查时显式加 `--dry-run --iterations 1 --launcher none`。Codex 或其他客户端仍可通过 `--agent-template 'codex --prompt "{target}"'` 或支持的 `{target_file}` 模板覆盖默认值。

当运行中的项目收到新的显式目标时，T0 不再只改 `.taskboard/targets/taskboard-T1.md` 文本；它会创建一个 `T1-方案需修改` intake TASK，让已经在 `/loop` 中的 T1 从 TASKBOARD 文件名自动接手。该 TASK 只携带用户目标和边界，不会预填 REQ、优先级、架构、验收或任务拆分。

执行 worker launcher 前，T0 默认会从 `--agent-template` 解析首个 agent command 并检查它是否在 PATH 中；默认 Claude 模板还会运行一次最小 `claude -p` readiness probe，而不是只相信 `claude auth status` 或 PATH。命令不存在会在启动 T1/T2/T3 前进入 `config-error`，持久化错误并要求修正 T0 配置。需要自定义 CLI/auth 检查时，使用 `--agent-preflight-command "<safe check command>"`，例如按当前客户端选择一个不会修改仓库的登录/版本/健康检查命令；该命令返回普通非零结果时错误文本会包含 `agent preflight command failed`。如果默认 readiness 或自定义 preflight 输出包含 `API Error: 403`、`Request not allowed`、`Failed to authenticate` 等托管子进程认证/权限拒绝线索，T0 会把 `agent_preflight.state` 标记为 `spawn-refused`，不再继续执行会失败的 launcher，而是直接写出下面的用户自有终端启动脚本。T0 可先运行 `python scripts/taskboard.py --root . launch-probe --launcher windows-terminal`，读取机器可读的 `recommended_backend`：`terminal` 表示使用 T0-managed terminal launcher，`subagent` 表示直接使用 native subagent fallback，`fix-config` 表示先修 T0 启动配置；这个探测是只读的，不能把 T1/T2/T3 管理转交给用户。高级用户可以用 `--no-agent-preflight` 或 `TASKBOARD_SKIP_DEFAULT_CLAUDE_READINESS=1` 关闭这一步，但默认建议保留，避免 Claude/Codex 等 agent CLI 未安装、未登录、托管子进程被拒绝或命令模板写错时把失败留在子终端里。
...
完整的原生子代理后备调度包会写入 `.taskboard/t0/subagent-fallback.json`，其 `kind` 为 `taskboard-subagent-fallback-packet`，包含可恢复派发的 `subagent_prompts`。`taskboard_progress.py` 会从 `.taskboard/t0/latest.json`、append-only event log 和这个 packet 文件恢复 `subagent_fallback_available`、`subagent_fallback_packet_available`、`subagent_fallback_packet_state`、`subagent_fallback_packet_file`、`subagent_prompt_count` 和 `subagent_prompt_roles`，因此 T0 重启或 latest snapshot 丢失时仍能拿回完整子代理派发材料，而不是退回让用户分别处理 worker。packet 会绑定生成时的 T0 goal；如果当前 `.taskboard/t0/goal.json` 与 packet goal 不一致，progress 会把它标为 `stale-goal`，`subagent plan` 不会派发旧 prompt。

T0 使用原生子代理后备调度时，不需要用户手动复制 prompts。T0 可以直接读取 `python scripts/taskboard_progress.py --root . --format json` 的 `subagent_spawn_request`；当 `action=spawn-native-subagent` 时，T0 用其中的 `prompt` 调用当前客户端的原生子代理工具，并用返回的 `ack_command` 记录 agent id、spawn tool、agent nickname 和 prompt hash。也可以运行 `python scripts/taskboard.py --root . subagent plan` 获取同一机器可读控制动作。子代理返回最终结果后，T0 用 `done_command` 或 `fail_command` 记录完成/失败，并附带 native wait/result metadata；需要重试失败角色时，用 `retry_command` 把旧记录归入 `attempts` 并重新进入 pending 队列。`python scripts/taskboard.py --root . subagent status` 会从 `.taskboard/t0/subagent-fallback.json` 和 `.taskboard/t0/subagents.json` 汇总 `pending_roles` / `active_roles` / `completed_roles` / `failed_roles`，让 T0 重启后继续派发或处理失败角色，而不是让用户接管。`taskboard_loop.py` 在生成 native-subagent fallback packet 后也会输出 `subagent_control`；外层集成可用 `--native-spawn-receipt-json` / `--native-spawn-receipt-file` 和 `--native-result-receipt-json` / `--native-result-receipt-file` 注入真实 native spawn/result receipt，loop 会自动写入 `subagent_ack` / `subagent_result` 并推进到下一角色计划。
真实 native-subagent 后端跑完后，用 `python scripts/taskboard_subagent_acceptance.py --root . --require-real-agent-ids --require-spawn-evidence --require-result-evidence` 验收证据。它会检查 fallback packet、T1/T2/T3 prompt 隔离契约、ack/result 记录、完成摘要、真实 agent id、`spawn_receipt`、`result_receipt`、`completion_receipt`、spawn tool、agent nickname、prompt_hash 和 summary_hash 一致性；smoke/test/manual 占位 id 会被拒绝，避免把脚本模拟误报为实战通过。
...
python scripts/taskboard.py --root . subagent plan
...
python scripts/taskboard.py --root . subagent done --role T1 --summary "<result>" --result-tool "<native wait tool>" --result-status "<native final status>"
python scripts/taskboard.py --root . subagent fail --role T1 --summary "<failure>" --result-tool "<native wait tool>" --result-status "<native final status>"
python scripts/taskboard.py --root . subagent retry --role T1 --note "<retry reason>"
python scripts/taskboard_loop.py --root . --native-spawn-receipt-json '{"role":"T1","agent_id":"<agent id>","spawn_tool":"<native spawn tool>","agent_nickname":"<agent nickname>"}'
python scripts/taskboard_loop.py --root . --native-result-receipt-json '{"role":"T1","summary":"<result>","result_tool":"<native wait tool>","result_status":"completed","native_status":"completed"}'
```
...
`taskboard_start.py` 使用和 supervisor loop 相同的运行态文件：默认写 `.taskboard/t0/latest.json`、`.taskboard/t0/events.jsonl` 和 `.taskboard/targets/taskboard-T*.md`。默认入口会把 `auto_mode`、`starter_mode`、`resume_config` 和 `launch_probe` 持久写入 latest snapshot，并把 `launch_probe_state`、`launch_probe_recommended_backend`、`launch_probe_reason` 写入 event log，再由 `taskboard_progress.py` 读出，方便恢复后确认当前 T0 是一键自动入口还是 dry check，并保留上次 T0 的 launcher、fallback launcher、agent template、agent preflight、lease、interval、target-file mode 和 terminal/subagent backend recommendation；恢复命令会保留 auto vs dry-check、`--fallback-launcher`、`--launcher none`、`--agent-preflight-command`、`--no-agent-preflight` 和 `--no-target-files`。需要 dry check 且不留下事件日志时使用 `--dry-run --no-event-log`。短跑验证时可以给 `--dry-run --iterations 1 --launcher none`，验证调度路径但不打开 worker 终端。
...
```bash
python scripts/taskboard_overnight_field_run.py --root . start --run-id "<field-run-id>"
python scripts/taskboard_overnight_field_run.py --root . resume
python scripts/taskboard_overnight_field_run.py --root . verify
...
```bash
python scripts/taskboard_t0.py --goal "完成 <你的开发目标>" --root . --launcher windows-terminal
```
...
The health check reports active queues, `.taskboard/alive` liveness, stalled tasks, the next role to wake, and manager-only actions. For a stalled TASK, an alive worker gets a target reissue; a missing/stale worker gets a T0-managed backend recovery. It does not let T0 perform design, review, implementation, verification, or commit work.
...
Liveness markers live under `.taskboard/alive/` and use file mtime only. `taskboard.py status` and `taskboard.py stall` read those markers so T0 can distinguish "worker loop is alive; reissue target" from "worker loop is missing/stale; recover the terminal or native subagent backend." Assignment heartbeat files live under `.taskboard/sessions/` and carry optional TASK acknowledgement fields. When a role is handling a concrete TASK file, it should include `--task` and `--assignment-id` so T0 can tell whether the dispatched work was acknowledged. These assignment fields are not task state, not shared role memory, and not a replacement for TASKBOARD filenames, history, or HANDOFF.

T0 supervisor loop entry:

```bash
python scripts/taskboard_loop.py --root . --goal "完成 <你的开发目标>" --forever --assignment-lease-seconds 300 --launcher windows-terminal
```
...
If the selected TASK file itself is older than `--stale-minutes`, T0 checks `.taskboard/alive` before acting: alive roles get a durable target reissue, while missing/stale roles record `stalled_recoveries` / `stalled_recovery_count` and recover the selected terminal or native-subagent backend in `--execute-launches` mode. This keeps stalled execution recovery inside T0 instead of asking the user to manage a worker terminal.
...
When real worker launch execution is active, T0 writes `.taskboard/t0/checkout-owner.json`
before worker launcher execution. `--launcher none` and no-launch checks do not hold this checkout-owner lock. A fresh marker from another top-level owner
suppresses worker launch with `checkout_owner_state=conflict`; the user should
wait for that owner or move the peer agent to a separate worktree, not manage
T1/T2/T3 manually. Use `--checkout-owner-id` for a stable orchestrator id and
`--checkout-owner-lease-seconds` to tune reclaim timing.
...
`taskboard_demo.py` 默认拒绝覆盖已有 `docs/`，适合用来演示 T0 如何从 demo TASKBOARD 队列中选择 T1/T2/T3，而不修改产品代码。`taskboard_e2e_smoke.py` 使用临时 TASKBOARD，跑一轮 T0 调度、模拟被选中的 worker `cycle` + heartbeat 认领任务，再归档 demo 任务、写 completion sentinel/dev-log，并确认 T0 progress 进入 `complete`；它用于验证“用户只给 T0 目标，T0 能把第一个 worker 管起来并收口完成证据”的控制面闭环，不会启动真实 Claude 或修改当前产品代码。`taskboard_cold_resume_smoke.py` 使用临时 git 仓库模拟隔夜 fresh worker：保存 T0 goal、活跃 T3 TASK、`Current Instruction`、unchecked Pending 和 scoped dirty file，然后验证 T0 从 TASKBOARD 状态恢复 T3，而不是要求用户管理 worker。`taskboard_cold_resume_acceptance.py` 用于真实项目冷恢复验收：它只读 `taskboard_progress.py` 输出，拒绝 smoke/test/demo 占位、缺失控制面证据和不完整 TASK 恢复证据。`taskboard_overnight_field_run.py` 记录真实隔夜 field run 的 start/resume/verify marker，作为 readiness 解除 `real overnight field run` gap 的证据。`taskboard_framework_readiness.py` 汇总全流程框架能力和剩余 field gap，防止把局部 smoke 或绿色单测误报为最终目标完成；它也会要求真实 native-subagent T1/T2/T3 记录通过 `taskboard_subagent_acceptance.py --require-real-agent-ids --require-spawn-evidence --require-result-evidence`。`taskboard_subagent_smoke.py` 使用临时 TASKBOARD，验证 T0 生成 native-subagent fallback packet 后能通过 `subagent next/ack/done/fail/retry` 走完 T1/T2/T3 后备派发状态机，并保留失败 attempt 历史。`taskboard_live_milestone_acceptance.py` 用于真实 field run 收口：它只读 `.taskboard/t0/latest.json`、`.taskboard/t0/events.jsonl`、T1/T2/T3 session/alive 证据、archive、STATE completion sentinel 和 dev-log，拒绝 smoke/test/manual 占位证据和 checkout-owner conflict，证明用户目标确实由 T0 管理到 milestone 完成。
...
`taskboard_t0_boundary_smoke.py` is a negative manager-boundary smoke: it runs
T0 dry-start in an isolated root and fails if T0 creates PROJECT/MAP/
REQUIREMENTS/STATE, TASK/archive files, source files, git files, or executed
worker launch commands. It proves T0 startup stays manager-only before worker
roles take over.
...
```bash
VERSION=v4.5.36 ./scripts/package.sh
```
...
- [ ] `python scripts/taskboard_e2e_smoke.py`
- [ ] `python scripts/taskboard_cold_resume_smoke.py`
- [ ] `python scripts/taskboard_cold_resume_acceptance.py --root <field-run-root>` before claiming a real cold worker resume is field-verified
- [ ] `python scripts/taskboard_overnight_field_run.py --root <field-run-root> start|resume|verify` before clearing the real overnight field-run gap
- [ ] `python scripts/taskboard_framework_readiness.py --root .` to inspect final-goal readiness and remaining field gaps
- [ ] `python scripts/taskboard_t0_boundary_smoke.py`
- [ ] `python scripts/taskboard_subagent_smoke.py`
- [ ] `python scripts/taskboard_subagent_acceptance.py --root <field-run-root> --require-real-agent-ids --require-spawn-evidence --require-result-evidence` when claiming real native-subagent backend evidence
- [ ] `python scripts/taskboard_live_milestone_acceptance.py --root <field-run-root>` before claiming a real T0-managed milestone is complete
...
- [ ] `python scripts/taskboard_stopgates.py --root .`
- [ ] `python scripts/taskboard_completion.py --root .`
- [ ] `python scripts/taskboard_live_milestone_acceptance.py --root <field-run-root>` when reporting field-run completion
- [ ] `python scripts/verify_t0_contract.py`
- [ ] `python -m unittest`
- [ ] `./scripts/package.sh`

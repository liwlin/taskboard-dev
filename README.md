...
`taskboard-dev` 是一个给 Claude Code / OpenAI Codex 等 agent CLI 使用的 **TASKBOARD 全自动流程开发 skill**。它把一次开发迭代拆成 T1 架构调度、T2 审核验证、T3 执行提交三个长期运行角色，通过文件名状态机驱动任务流转，适合 5–20 个任务规模的 milestone / feature batch。
...
- **长任务目标驱动**：推荐先给每个角色一条 `目标 / target` 指令，再运行 `/taskboard-dev T{N}`。
...
```text
dist/taskboard-dev-v4.2.tar.gz
dist/taskboard-dev-v4.2.zip
```
...
打开 3 个 agent CLI 终端，**先写目标指令，再执行角色命令**。
...
```bash
VERSION=v4.2 ./scripts/package.sh
```

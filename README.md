...
For longer unattended sessions, T0 can run bounded guardian cycles that repeatedly check and resume only the T0 supervisor:

```bash
python scripts/taskboard_watchdog.py --root . --guardian --execute
```

This returns `taskboard-t0-guardian`; it reduces reliance on manually re-running the watchdog while still preserving the rule that T0 does not directly manage worker tasks.

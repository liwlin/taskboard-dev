...
```text
dist/taskboard-dev-v4.5.10.tar.gz
dist/taskboard-dev-v4.5.10.zip
```
...
T0 使用原生子代理后备调度时，不需要用户手动复制 prompts。T0 先运行 `python scripts/taskboard.py --root . subagent plan` 获取单一机器可读控制动作；当 `action=spawn-native-subagent` 时，T0 用返回的 `prompt` 调用当前客户端的原生子代理工具，并用返回的 `ack_command` 记录 agent id、spawn tool、agent nickname 和 prompt hash。子代理返回最终结果后，T0 用 `done_command` 或 `fail_command` 记录完成/失败；需要重试失败角色时，用 `retry_command` 把旧记录归入 `attempts` 并重新进入 pending 队列。`python scripts/taskboard.py --root . subagent status` 会从 `.taskboard/t0/subagent-fallback.json` 和 `.taskboard/t0/subagents.json` 汇总 `pending_roles` / `active_roles` / `completed_roles` / `failed_roles`，让 T0 重启后继续派发或处理失败角色，而不是让用户接管。`taskboard_loop.py` 在生成 native-subagent fallback packet 后也会输出 `subagent_control`；如果外层集成注入真实 native spawn receipt，loop 会自动写入 `subagent_ack` 并推进到下一角色计划。
真实 native-subagent 后端跑完后，用 `python scripts/taskboard_subagent_acceptance.py --root . --require-real-agent-ids --require-spawn-evidence` 验收证据。它会检查 fallback packet、T1/T2/T3 prompt 隔离契约、ack/result 记录、完成摘要、真实 agent id、`spawn_receipt`、`completion_receipt`、spawn tool、agent nickname 和 prompt_hash 一致性；smoke/test/manual 占位 id 会被拒绝，避免把脚本模拟误报为实战通过。
...
```bash
VERSION=v4.5.10 ./scripts/package.sh
```
...
- [ ] `python scripts/taskboard_t0_boundary_smoke.py`
- [ ] `python scripts/taskboard_subagent_smoke.py`
- [ ] `python scripts/taskboard_subagent_acceptance.py --root <field-run-root> --require-real-agent-ids --require-spawn-evidence` when claiming real native-subagent backend evidence

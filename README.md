...
```text
dist/taskboard-dev-v4.5.21.tar.gz
dist/taskboard-dev-v4.5.21.zip
```
...
该 recorder 只写 `.taskboard/t0/overnight-field-run.json` 这一份 T0 控制面证据。`start` 要求当前真实冷恢复验收通过，`resume` 要求第二天重开 worker 后再次通过冷恢复验收并满足 elapsed 门槛，`verify` 要求 live milestone acceptance 通过，`status` 只读显示当前阶段、下一条命令、下一关卡是否 ready 以及 blocker 列表。它不会启动 worker、编辑 TASKBOARD 任务、实现代码或制造 T1/T2/T3 证据。
...
```bash
python scripts/taskboard_t0.py --goal "完成 <你的开发目标>" --root . --launcher windows-terminal --agent-template 'claude "{target}"'
```
...
```bash
python scripts/taskboard_loop.py --root . --goal "完成 <你的开发目标>" --forever --assignment-lease-seconds 300 --launcher windows-terminal --agent-template 'claude "{target}"'
```
...
When `--execute-launches` is active, T0 writes `.taskboard/t0/checkout-owner.json`
before worker launcher execution. A fresh marker from another top-level owner
...
```bash
VERSION=v4.5.21 ./scripts/package.sh
```

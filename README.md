...
```text
dist/taskboard-dev-v4.5.8.tar.gz
dist/taskboard-dev-v4.5.8.zip
```
...
真实 native-subagent 后端跑完后，用 `python scripts/taskboard_subagent_acceptance.py --root . --require-real-agent-ids` 验收证据。它会检查 fallback packet、T1/T2/T3 prompt 隔离契约、ack/result 记录、完成摘要和真实 agent id；smoke/test/manual 占位 id 会被拒绝，避免把脚本模拟误报为实战通过。
...
```bash
VERSION=v4.5.8 ./scripts/package.sh
```
...
- [ ] `python scripts/taskboard_t0_boundary_smoke.py`
- [ ] `python scripts/taskboard_subagent_smoke.py`
- [ ] `python scripts/taskboard_subagent_acceptance.py --root <field-run-root> --require-real-agent-ids` when claiming real native-subagent backend evidence

...
└── scripts/
    ...
    ├── taskboard_e2e_smoke.py       # 验证 T0 目标到 worker 认领的闭环
...
```text
dist/taskboard-dev-v4.5.1.tar.gz
dist/taskboard-dev-v4.5.1.zip
```
...
`taskboard_demo.py` 默认拒绝覆盖已有 `docs/`，适合用来演示 T0 如何从 demo TASKBOARD 队列中选择 T1/T2/T3，而不修改产品代码。`taskboard_e2e_smoke.py` 使用临时 TASKBOARD，跑一轮 T0 调度、模拟被选中的 worker `cycle` + heartbeat 认领任务，再跑一轮 T0 确认 assignment acknowledged；它用于验证“用户只给 T0 目标，T0 能把第一个 worker 管起来”的控制面闭环，不会启动真实 Claude 或修改当前产品代码。
...
```bash
VERSION=v4.5.1 ./scripts/package.sh
```

...
- Real overnight worker-terminal field runs should be recorded with `python scripts/taskboard_overnight_field_run.py --root . start --run-id "<field-run-id>"`, then `resume`, then `verify`; use `python scripts/taskboard_overnight_field_run.py --root . status` to see the current stage, next command, next gate readiness, blockers, and T0 prepare command when a gate is blocked. It only writes `.taskboard/t0/overnight-field-run.json` as T0 control-plane evidence and must not launch workers or create T1/T2/T3 proof.

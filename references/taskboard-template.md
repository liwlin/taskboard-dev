...
- Real overnight worker-terminal field runs should be recorded with `python scripts/taskboard_overnight_field_run.py --root . start --run-id "<field-run-id>"`, then `resume`, then `verify`; use `python scripts/taskboard_overnight_field_run.py --root . status` to see the current stage, next command, next gate readiness, blockers, and T0 prepare command when a gate is blocked. The status `next_command` omits `--run-id` for the `start` stage so the recorder can auto-generate a run id; explicit `--run-id` remains available for manually named field runs. If `.taskboard/t0/goal.json` already stores the user goal, status reuses it in prepare command; otherwise it leaves a `<user goal>` placeholder. It only writes `.taskboard/t0/overnight-field-run.json` as T0 control-plane evidence and must not launch workers or create T1/T2/T3 proof.

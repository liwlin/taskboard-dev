...
- `taskboard_progress.py` reports `resume_command`, `t0_supervisor_state`, queue metrics, assignment state, latest event recovery data, completion audit state, fallback launcher state, stalled recovery state, and `cold_resume_readiness` for the current worker TASK.
...
- Real overnight worker-terminal field runs should be recorded with `python scripts/taskboard_overnight_field_run.py --root . start --run-id "<field-run-id>"`, then `resume`, then `verify`; use `python scripts/taskboard_overnight_field_run.py --root . status` to see the current stage, next command, next gate readiness, blockers, and T0 prepare command when a gate is blocked. The status `next_command` omits `--run-id` for the `start` stage so the recorder can auto-generate a run id; explicit `--run-id` remains available for manually named field runs. If `.taskboard/t0/goal.json` already stores the user goal, status reuses it in prepare command; otherwise `status --goal "<user goal>"` can render a placeholder-free prepare command without writing the goal or creating field-run evidence. It only writes `.taskboard/t0/overnight-field-run.json` as T0 control-plane evidence and must not launch workers or create T1/T2/T3 proof.
- Framework readiness claims should use `python scripts/taskboard_framework_readiness.py --root .`; it reports `field-verification-required` until a real overnight worker-terminal field run is validated by `taskboard_overnight_field_run.py` and a real native-subagent T1/T2/T3 run passes `taskboard_subagent_acceptance.py --require-real-agent-ids --require-spawn-evidence --require-result-evidence`; it must remain read-only.
...
- Native subagent fallback is also handled inside T0: T0 uses `taskboard.py subagent plan` to get one machine-readable control action, dispatches the returned prompt through the current client's native subagent tool when `action=spawn-native-subagent`, records the returned agent id with the plan's `ack_command`, records final results with `done_command` / `fail_command` plus native wait/result metadata, and uses `retry_command` to archive a failed attempt before requeueing that role. `taskboard_loop.py` emits `subagent_control` after writing a fallback packet and can record injected native spawn/result receipts as `subagent_ack` / `subagent_result` through `--native-spawn-receipt-json` / `--native-spawn-receipt-file` and `--native-result-receipt-json` / `--native-result-receipt-file`; `taskboard_progress.py` exposes `subagent_control_state`, `subagent_control_next_role`, `subagent_plan_command`, `subagent_next_command`, `subagent_ack_command`, `subagent_done_command`, `subagent_fail_command`, and `subagent_retry_commands` so T0 can resume the correct native subagent control action after restart. Restarts continue pending/active/failed roles instead of asking the user to manage T1/T2/T3.
- Real native-subagent backend claims must pass `python scripts/taskboard_subagent_acceptance.py --root . --require-real-agent-ids --require-spawn-evidence --require-result-evidence`; the check requires `spawn_receipt`, `result_receipt`, `completion_receipt`, prompt_hash consistency, and summary_hash consistency, while the smoke script alone proves bookkeeping, not live native-subagent execution.

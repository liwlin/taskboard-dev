...
- Framework readiness claims should use `python scripts/taskboard_framework_readiness.py --root .`; it reports `field-verification-required` until a real overnight worker-terminal field run is validated by `taskboard_overnight_field_run.py`, and it must remain read-only.

...
- If `--agent-preflight-command` or an executed launcher command reports managed child-process auth/permission refusal such as `API Error: 403`, `Request not allowed`, or `Failed to authenticate`, classify it as `agent_preflight.state=spawn-refused` or a spawn-refusal launch failure. Do not keep executing doomed launcher commands. Write `.taskboard/open-tabs.ps1` and `.taskboard/launch-role.ps1` so the user performs one T0-directed startup action from their own authenticated terminal.

...
- `scripts/taskboard_start.py --goal "<user goal>"` is the one-command T0 entry point and is the default user-facing entry for actual T0 supervision: it executes managed-role launch/recovery commands and keeps supervising until completion, a stop gate, a missing goal, a configuration error, or interruption. It defaults to `--launcher windows-terminal` and a file-backed Claude launch script: T0 writes `.taskboard/targets/taskboard-T*.md` plus `.taskboard/targets/taskboard-T*.launch.ps1`, and the worker command uses `powershell -File` so the script passes Claude a short target-file pointer prompt instead of the full target body. The worker must read the referenced UTF-8 target file before acting. Use `--dry-run --iterations 1 --launcher none` only for short verification runs that must not open worker terminals.

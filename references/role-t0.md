...
- `scripts/taskboard_start.py --goal "<user goal>"` is the one-command T0 entry point and is the default user-facing entry for actual T0 supervision: it executes managed-role launch/recovery commands and keeps supervising until completion, a stop gate, a missing goal, a configuration error, or interruption. It defaults to `--launcher windows-terminal` and a file-backed Claude launch script: T0 writes `.taskboard/targets/taskboard-T*.md` plus `.taskboard/targets/taskboard-T*.launch.ps1`, and the worker command uses `powershell -File` so the script bootstrap-refreshes `.taskboard/alive/T{N}` and passes Claude a short target-file pointer prompt instead of the full target body. The bootstrap alive marker proves the managed role terminal started; it is not TASK acknowledgement. The worker must read the referenced UTF-8 target file before acting. Use `--dry-run --iterations 1 --launcher none` only for short verification runs that must not open worker terminals.
...
- `--agent-template` is the client-specific command T0 runs inside each role terminal. It supports `{role}`, `{title}`, `{command}`, `{target}`, and `{target_file}` placeholders. The default Claude template is an internal file-backed sentinel; T0 renders it as a per-role PowerShell launch script that passes a short pointer prompt instead of passing the multi-kilobyte target inline or relying on compound `-Command` parsing. When a launcher command actually references `{target_file}`, `scripts/taskboard_t0.py` writes the corresponding role target files and returns a `target_files` list; inline `{target}` launchers remain available for clients that explicitly want inline prompts, and dry checks do not write those runtime files.

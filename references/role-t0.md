...
- Before claiming real native-subagent backend execution, T0 MUST run `python scripts/taskboard_subagent_acceptance.py --root . --require-real-agent-ids`. This acceptance check verifies the fallback packet, per-role prompt isolation gates, ack/result records, completion summaries, and non-placeholder agent ids; `taskboard_subagent_smoke.py` alone proves bookkeeping, not live native-subagent execution.

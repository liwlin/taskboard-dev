...
Use `python scripts/taskboard_watchdog.py --root . --guardian --execute` for bounded guardian mode when T0 should repeatedly check and resume only the T0 supervisor. Guardian mode returns `taskboard-t0-guardian` and must not launch or manage T1/T2/T3 directly.

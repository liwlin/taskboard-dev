...
- When an explicit `--goal` changes after a saved goal already exists, create a queue-visible `T1-方案需修改` intake TASK so a running T1 `/loop` can pick up the new requirement from TASKBOARD filenames. This TASK is only an intake event; it must not contain T0-authored REQ skeletons, priorities, design choices, task splits, acceptance criteria, or file lists.
...
python scripts/taskboard_loop.py --root . --goal "<user goal>" --forever --assignment-lease-seconds 300 --launcher windows-terminal
python scripts/taskboard_t0.py --goal "<user goal>" --root . --launcher windows-terminal
python scripts/taskboard_t0.py --goal "<user goal>" --root . --launcher powershell
python scripts/taskboard_t0.py --goal "<user goal>" --root . --launcher tmux
```
...
- `scripts/taskboard_start.py --goal "<user goal>"` is the one-command T0 entry point and is the default user-facing entry for actual T0 supervision: it executes managed-role launch/recovery commands and keeps supervising until completion, a stop gate, a missing goal, a configuration error, or interruption. It defaults to `--launcher windows-terminal` and a file-backed Claude launch script: T0 writes `.taskboard/targets/taskboard-T*.md` plus `.taskboard/targets/taskboard-T*.launch.ps1`, and the worker command uses `powershell -File` so the script clears inherited `CLAUDE*` environment variables, detects Git Bash into `CLAUDE_CODE_GIT_BASH_PATH`, bootstrap-refreshes `.taskboard/alive/T{N}` using the bundled skill script path, records a launched session heartbeat, exports `TASKBOARD_TARGET_FILE`, and starts Claude with `/loop 2m /taskboard-dev T{N}` plus `--dangerously-skip-permissions` instead of a one-shot pointer prompt. The bootstrap alive marker proves the managed role terminal started; it is not TASK acknowledgement. The worker must read the referenced UTF-8 target file before acting. Use `--dry-run --iterations 1 --launcher none` only for short verification runs that must not open worker terminals.
...
- `--agent-template` is the client-specific command T0 runs inside each role terminal. It supports `{role}`, `{title}`, `{command}`, `{target}`, and `{target_file}` placeholders. The default Claude template is an internal file-backed sentinel; T0 renders it as a per-role PowerShell launch script that passes only the loop command to Claude, while `TASKBOARD_TARGET_FILE` points the worker at the multi-kilobyte target file and avoids compound `-Command` parsing. When a launcher command actually references `{target_file}`, `scripts/taskboard_t0.py` writes the corresponding role target files and returns a `target_files` list; inline `{target}` launchers remain available for clients that explicitly want inline prompts, and dry checks do not write those runtime files.
...
- If a new explicit `--goal` differs from the saved T0 goal, T0 writes one queue-visible T1 intake task and refreshes role targets. If `docs/STATE.md` still has `Goal Complete: yes/true`, T0 also treats that sentinel as stale, rewrites it to `Goal Complete: no`, and records `.taskboard/t0/goal-reset.json`. Do not ask the user to hand-edit target files or paste prompts into T1 after a completed milestone or added requirement.
...
- Use `scripts/taskboard_loop.py` for the actual T0 supervisor loop. It combines session heartbeat probing, queue health, and dispatch into each iteration. Add `--execute-launches` only when T0 should execute generated launcher commands; execute mode runs only missing/stale role recovery commands and must not relaunch healthy roles just because the dispatch plan contains full starter commands. Before relaunching or killing a worker, T0 must treat recent alive mtime, session heartbeat, TASK mtime, or visible CPU/process activity as evidence that the role may still be working; do not kill an active role merely because no output file has appeared yet. Those commands only launch/recover T1/T2/T3 and must not perform worker tasks in T0.
...
- Cross-day cold resume: cold start is the default correctness path. Terminal identity is role-bound; topic identity is TASK-bound. When a project is reopened the next day, T0 restores the goal from `.taskboard/t0/goal.json`, regenerates current `.taskboard/targets/taskboard-T*.md`, and lets fresh worker terminals recover topic from TASKBOARD filenames, stable docs, the current TASK file, history, unchecked Pending items, `Current Instruction`, and scoped `git status`. `claude --resume` is allowed only as an optimization for the same role, same TASK, and same TASK version. Never treat a resumed chat session as authoritative; if resumed chat conflicts with TASKBOARD state or the TASK version changed, discard chat context and cold-start from the board. After every Pending item completion, update the TASK immediately: check the item, append or prepare the history note, and refresh Current Instruction to the next concrete step before continuing. Workers should write a one-line `Current Instruction` before pause, shutdown, or context-limit restart so the next fresh terminal has a precise next step.

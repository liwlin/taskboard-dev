...
1. **托管会话内 spawn 必 403**：T0 自动拉起 worker 终端的主路径在托管
   Claude Code 会话中结构性失效（三次受控测试证实，认证不可继承）。
...
```
探测：spawn 自检（headless claude -p "ping"）
  ├─ 成功 → terminal 后端：生成 open-tabs.ps1 交用户执行（或直接执行），
  │         worker 常驻循环
  └─ 403  → subagent 后端：T0 逐任务派隔离子代理，自己只读监控
```

托管会话是默认假设，terminal 是增强（C2，与 v4.3 假设相反）。
两个后端共享同一套调度逻辑（T0 next 优先级、stall 响应、停止门聚合）。
